...
export default function PersonalAttendance({ user, initialMonth, initialRecords, embedded = false }: PersonalAttendanceProps) {
  ...
  const content = (
    <>
      ...
      <section className="overflow-visible border border-stone-300 bg-[#fffdf8] shadow-[0_8px_30px_rgba(41,37,36,0.05)]">
        <div className="flex flex-col gap-3 border-b border-stone-200 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-bold text-stone-600"><Legend tone="bg-emerald-500" label="Đi làm" /><Legend tone="bg-sky-500" label="Tăng ca" /><Legend tone="bg-amber-500" label="Trễ / về sớm" /><Legend tone="bg-rose-500" label="Nghỉ" /></div>
          <div className="flex items-center justify-between gap-2 sm:justify-end"><button aria-label="Tháng trước" onClick={() => changeMonth(-1)} className="grid h-9 w-9 place-items-center border border-stone-300 bg-white hover:border-orange-500 hover:text-orange-600"><ChevronLeft className="h-4 w-4" /></button><p className="min-w-32 text-center text-sm font-black">Tháng {monthIndex}/{year}</p><button aria-label="Tháng sau" onClick={() => changeMonth(1)} className="grid h-9 w-9 place-items-center border border-stone-300 bg-white hover:border-orange-500 hover:text-orange-600"><ChevronRight className="h-4 w-4" /></button></div>
        </div>

        {error ? <div className="m-4 border border-rose-200 bg-rose-50 p-4 text-sm font-semibold text-rose-700">{error}</div> : (
          <div className="grid items-start xl:grid-cols-[minmax(0,1fr)_360px]">
            <div className="relative p-3 sm:p-4 xl:border-r xl:border-stone-200">
              <div className="grid grid-cols-7 gap-1.5 sm:gap-2">
                {weekDays.map((day) => <div key={day} className="py-2 text-center text-[10px] font-black uppercase tracking-[0.16em] text-stone-400">{day}</div>)}
              ...
              {isLoading && <div className="pointer-events-none absolute inset-0 grid place-items-center bg-[#fffdf8]/70 backdrop-blur-[1px]"><span className="border border-stone-800 bg-stone-950 px-4 py-2 text-xs font-black text-white">Đang tải tháng {monthIndex}/{year}</span></div>}
            </div>
...
function AttendanceInspector({ record }: { record: AttendanceRecord | null }) {
  if (!record) return <div className="grid min-h-[560px] place-items-center p-8 text-center"><div><div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-orange-100 text-orange-600"><Clock3 className="h-5 w-5" /></div><p className="mt-4 font-black text-stone-900">Chi tiết trong ngày</p><p className="mt-1 text-sm leading-6 text-stone-500">Rê chuột vào một ô có màu.<br />Thông tin sẽ hiện ngay tại đây.</p></div></div>;
  const state = getAttendanceState(record);
  return <div className="p-5"><div className="flex items-start justify-between gap-3 border-b border-stone-200 pb-4"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-orange-600">Ngày đang xem</p><h2 className="mt-1 text-2xl font-black text-stone-950">{formatCalendarDate(record.workDate)}</h2><p className="mt-1 text-xs text-stone-500">{record.weekdayName || "--"} · {record.shiftName || "Chưa có ca"}</p></div><span className={`px-2.5 py-1 text-[10px] font-black uppercase ${state.badgeClass}`}>{state.label}</span></div><InspectorContent record={record} /></div>;
}
...
function MobileAttendanceDrawer({ record, onClose }: { record: AttendanceRecord; onClose: () => void }) { return <div className="fixed inset-0 z-50 flex items-end bg-stone-950/50 xl:hidden" onClick={onClose}><div className="max-h-[88vh] w-full overflow-y-auto rounded-t-3xl bg-[#fffdf8] p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between gap-3 border-b border-stone-200 pb-4"><div><p className="text-xs font-black uppercase tracking-wide text-orange-600">Chi tiết chấm công</p><h2 className="mt-1 text-2xl font-black">{formatCalendarDate(record.workDate)}</h2><p className="text-sm text-stone-500">{record.shiftName || "Chưa có ca làm"}</p></div><button aria-label="Đóng" onClick={onClose} className="grid h-9 w-9 place-items-center rounded-full bg-stone-200"><X className="h-5 w-5" /></button></div><InspectorContent record={record} /></div></div>; }

function HeaderMetric({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) { return <div className="border-l-2 border-orange-500 bg-[#fffdf8] px-3 py-2.5 shadow-sm"><span className="text-orange-600 [&>svg]:h-4 [&>svg]:w-4">{icon}</span><p className="mt-1 text-[9px] font-black uppercase tracking-[0.12em] text-stone-400">{label}</p><p className="text-lg font-black text-stone-950">{value}</p></div>; }
function DetailSection({ title, children }: { title: string; children: React.ReactNode }) { return <section><h3 className="mb-3 text-[10px] font-black uppercase tracking-[0.16em] text-stone-400">{title}</h3>{children}</section>; }
function TimePair({ label, checkIn, checkOut }: { label: string; checkIn: string | null; checkOut: string | null }) { return <div className="bg-stone-100 p-2.5 text-center"><p className="text-[9px] font-black uppercase text-stone-400">{label}</p><p className="mt-1 text-xs font-black text-stone-900">{time(checkIn)}–{time(checkOut)}</p></div>; }
function DetailValue({ label, value }: { label: string; value: string }) { return <div><p className="text-[10px] text-stone-400">{label}</p><p className="mt-0.5 text-sm font-black text-stone-900">{value}</p></div>; }
function DetailRow({ label, value }: { label: string; value: string }) { return <div className="flex items-start justify-between gap-4 text-xs"><span className="text-stone-500">{label}</span><span className="text-right font-bold text-stone-900">{value}</span></div>; }
