...
function DialogFrame({ title, subtitle, onClose, wide = false, children }: { title: string; subtitle: string; onClose: () => void; wide?: boolean; children: React.ReactNode }) {
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-stone-950/60 p-0 backdrop-blur-sm sm:items-center sm:p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div role="dialog" aria-modal="true" aria-label={title} className={`max-h-[92vh] w-full overflow-y-auto rounded-t-3xl bg-[#fffdf8] shadow-2xl sm:rounded-none sm:border sm:border-stone-400 ${wide ? "max-w-4xl" : "max-w-xl"}`}><header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-stone-300 bg-[#f3f0e9]/95 px-5 py-4 backdrop-blur sm:px-6"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-orange-600">Personal workspace</p><h2 className="mt-1 text-xl font-black text-stone-950">{title}</h2><p className="mt-1 text-sm text-stone-500">{subtitle}</p></div><button aria-label="Đóng" onClick={onClose} className="grid h-9 w-9 shrink-0 place-items-center bg-stone-950 text-white hover:bg-orange-600"><X className="h-5 w-5" /></button></header><div className="p-5 sm:p-6">{children}</div></div></div>;
}
