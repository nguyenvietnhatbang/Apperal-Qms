    ...
    <section className="border-b border-slate-300 pb-5">
      <div className="grid gap-4 sm:grid-cols-[1fr_auto] sm:items-end">
        <div className="flex min-w-0 items-center gap-4">
          <div className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-emerald-600 text-xl font-black text-white shadow-sm">
            {overview.profile.fullName.slice(0, 1).toUpperCase()}
          </div>
          <div className="min-w-0">
            <p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-600">Personal finance workspace</p>
            <h1 className="mt-1 truncate text-3xl font-black tracking-tight text-slate-950">{overview.profile.fullName}</h1>
        ...
        <div className="flex items-center gap-3 rounded-xl border-l-2 border-emerald-500 bg-white px-4 py-3 shadow-sm">
          <Clock3 className="h-5 w-5 text-emerald-600" />
          <div>
            <p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Chấm công gần nhất</p>
...
function PayrollWorkspace({
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-slate-300 bg-white shadow-[0_8px_30px_rgba(15,23,42,0.06)]">
      <header className="border-b border-slate-200 bg-[#f1f5f9] p-4 sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-700">Income statement</p>
            <h2 className="mt-1 text-xl font-black text-slate-950">Bảng lương theo kỳ</h2>
          </div>
          <select
            aria-label="Chọn kỳ lương"
            value={selectedCycleKey}
            onChange={(event) => onCycleChange(event.target.value)}
            className="w-full rounded-xl border border-slate-400 bg-white px-3 py-2.5 text-sm font-bold outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 sm:max-w-[320px]"
          >
      ...
      {!selectedPayroll ? (
        ...
        <div className="p-4 sm:p-5">
          ...
          {selectedPayroll.note && <p className="mt-4 bg-slate-100 px-4 py-3 text-sm text-slate-600">Ghi chú: {selectedPayroll.note}</p>}
          ...
          <button
            ...
            className={`mt-4 flex w-full items-center justify-center gap-2 px-4 py-3 text-sm font-black transition disabled:opacity-60 ${isPending ? "bg-amber-500 text-slate-950 hover:bg-amber-400" : "bg-slate-950 text-white hover:bg-emerald-600"}`}
          >
...
function SalaryConfigurationCard({ overview, onOpen }: { overview: PersonalOverview; onOpen: () => void }) {
  ...
  return (
    <section className="rounded-xl border border-slate-300 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.15em] text-slate-400">Salary setup</p><h2 className="mt-1 text-lg font-black">Mức lương áp dụng</h2></div><CircleDollarSign className="h-6 w-6 text-emerald-600" /></div>
      {config ? <><p className="mt-4 text-3xl font-black text-slate-950">{formatVND(config.totalSalary)} <span className="text-base">đ</span></p><p className="mt-1 text-xs text-slate-500">Hiệu lực từ {formatDate(config.effectiveFrom)}</p><div className="mt-4 space-y-2 border-t border-slate-200 pt-4"><CompactLine label="Lương cơ bản" value={config.baseSalary} /><CompactLine label="Lương đóng bảo hiểm" value={config.insuranceSalary} /><CompactLine label="Tổng phụ cấp" value={Number(config.totalSalary) - Number(config.baseSalary)} /></div><button onClick={onOpen} className="mt-4 w-full rounded-xl border border-slate-400 px-4 py-2.5 text-sm font-black hover:border-emerald-500 hover:bg-emerald-50 hover:text-emerald-700">Xem toàn bộ cấu hình</button></> : <p className="mt-4 text-sm text-slate-500">Chưa có cấu hình lương đang hiệu lực.</p>}
    </section>
  );
}
...
function EmployeeInformationCard({ overview }: { overview: PersonalOverview }) {
  return (
    <section className="rounded-xl border border-slate-300 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.15em] text-slate-400">Employment profile</p><h2 className="mt-1 text-lg font-black">Hồ sơ nhân viên</h2></div><UserRound className="h-6 w-6 text-emerald-600" /></div>
...
function WorkMetric({ label, value }: { label: string; value: string }) {
  return <div className="bg-white p-3"><p className="text-[10px] font-bold text-slate-500">{label}</p><p className="mt-1 text-sm font-black text-slate-950">{value}</p></div>;
}
...
function SalaryBreakdown({ title, icon, rows, danger = false }: { title: string; icon: React.ReactNode; rows: readonly (readonly [string, string | number | undefined])[]; danger?: boolean }) {
  return <div className="rounded-xl border border-slate-200 p-4"><div className="flex items-center gap-2 text-sm font-black text-slate-950">{icon}{title}</div><div className="mt-3 space-y-2.5">{rows.map(([label, value]) => <div key={label} className="flex items-center justify-between gap-3 text-xs"><span className="text-slate-600">{label}</span><span className={danger && Number(value || 0) > 0 ? "font-black text-rose-600" : "font-black text-slate-950"}>{formatVND(value || 0)} đ</span></div>)}</div></div>;
}
...
function InfoLine({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="flex items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center bg-slate-100 text-slate-500 [&>svg]:h-4 [&>svg]:w-4">{icon}</span><div className="min-w-0"><p className="text-xs text-slate-500">{label}</p><p className="truncate font-bold text-slate-950">{value}</p></div></div>;
}
