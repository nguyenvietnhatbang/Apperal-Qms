...
export default function PersonalShell({ user, children, activeView, onViewChange }: { user: { displayName: string; factoryName: string }; children: React.ReactNode; activeView?: PersonalView; onViewChange?: (view: PersonalView) => void }) {
  ...
  return <main className="min-h-screen bg-[#f3f0e9] text-stone-950 lg:flex">
    ...
    <div className="min-w-0 flex-1">
      ...
      <nav className="fixed inset-x-3 bottom-3 z-40 grid grid-cols-2 border border-stone-300 bg-[#fffdf8]/95 p-1 shadow-2xl backdrop-blur lg:hidden"><MobileNav href="/personal" active={overviewActive} icon={<LayoutGrid />} label="Tổng quan" onActivate={onViewChange ? () => onViewChange("overview") : undefined} /><MobileNav href="/personal/attendance" active={!overviewActive} icon={<CalendarDays />} label="Chấm công" onActivate={onViewChange ? () => onViewChange("attendance") : undefined} /></nav>
    </div>
  </main>;
}

function NavLink({ href, active, icon, label, onActivate }: { href: string; active: boolean; icon: React.ReactNode; label: string; onActivate?: () => void }) { const className = `flex w-full items-center gap-3 px-3 py-3 text-left text-sm font-black transition [&>svg]:h-4 [&>svg]:w-4 ${active ? "bg-orange-500 text-stone-950" : "text-stone-400 hover:bg-white/5 hover:text-white"}`; return onActivate ? <button type="button" onClick={onActivate} className={className}>{icon}{label}</button> : <Link href={href} className={className}>{icon}{label}</Link>; }
function MobileNav({ href, active, icon, label, onActivate }: { href: string; active: boolean; icon: React.ReactNode; label: string; onActivate?: () => void }) { const className = `flex items-center justify-center gap-2 px-3 py-2.5 text-xs font-black [&>svg]:h-4 [&>svg]:w-4 ${active ? "bg-stone-950 text-white" : "text-stone-500"}`; return onActivate ? <button type="button" onClick={onActivate} className={className}>{icon}{label}</button> : <Link href={href} className={className}>{icon}{label}</Link>; }
